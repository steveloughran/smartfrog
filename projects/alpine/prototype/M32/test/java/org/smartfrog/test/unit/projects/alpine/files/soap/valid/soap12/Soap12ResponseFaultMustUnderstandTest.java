package org.smartfrog.test.unit.projects.alpine.files.soap.valid.soap12;

import org.smartfrog.test.unit.projects.alpine.ValidTestBase;

/**

 */
public class Soap12ResponseFaultMustUnderstandTest extends ValidTestBase {
    public Soap12ResponseFaultMustUnderstandTest(String name) {
        super(name);
    }

    /**
     * Implement this
     *
     * @return the resource to test
     */
    protected String getTestResource() {
        return SOAP12_RESPONSE_FAULT_MUST_UNDERSTAND;
    }
}
