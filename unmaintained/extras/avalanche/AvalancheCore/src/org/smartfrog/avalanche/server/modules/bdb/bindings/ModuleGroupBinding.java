...
public class ModuleGroupBinding extends TupleBinding{

	public Object entryToObject(TupleInput data) {
		ModuleGroupDocument mgdoc = null ;
		String text = data.readString();
		try{
			 mgdoc = ModuleGroupDocument.Factory.parse(text);
	...
	public Object entryToObject(DatabaseEntry data) {
		...
		if( null != dataBytes ){
			try{
				 mgdoc = ModuleGroupDocument.Factory.parse(new String(dataBytes));
			}catch(Exception e){
				// wont happen unless db goes bad.
				e.printStackTrace();
			}
	...
	public void objectToEntry(Object obj, TupleOutput data) {
		ModuleGroupDocument mgdoc = (ModuleGroupDocument)obj;
		data.writeString(mgdoc.xmlText());
	}

	public void objectToEntry(Object obj, DatabaseEntry data) {
		ModuleGroupDocument mgdoc = (ModuleGroupDocument)obj;
		data.setData(mgdoc.xmlText().getBytes());
	}
}
