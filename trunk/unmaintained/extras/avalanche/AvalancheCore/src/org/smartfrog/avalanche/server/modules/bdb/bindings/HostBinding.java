...
public class HostBinding extends TupleBinding {

	public Object entryToObject(TupleInput data) {
		String text = data.readString();
		HostDocument hdoc = null ;
		try{
			hdoc = HostDocument.Factory.parse(text);
		}catch(Exception e){
			// wont happen
			e.printStackTrace();
		}
	...
	public Object entryToObject(DatabaseEntry data) {
		...
		if( null != dataBytes ){
			try{
				hdoc = HostDocument.Factory.parse(new String(dataBytes));
			}catch(Exception e){
				// wont happen
				e.printStackTrace();
			}
	...
	public void objectToEntry(Object obj, TupleOutput data) {
		HostDocument hdoc = (HostDocument)obj;
		data.writeString(hdoc.xmlText());
	}
	public void objectToEntry(Object obj, DatabaseEntry data) {
		HostDocument hdoc = (HostDocument)obj;
		data.setData(hdoc.xmlText().getBytes());
	}
