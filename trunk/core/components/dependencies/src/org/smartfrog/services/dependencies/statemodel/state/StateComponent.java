    ...
    public void go(String transition) throws StateComponentTransitionException {
        ...
        try {
            if (eventLog != null) {
                ...
                eventLog.sfReplaceAttribute("transition" + count, fullName + ":" + transition);
                eventLog.sfReplaceAttribute(COUNT, ++count);
            }
        } catch (Exception e) {
        	sfLog().error(StateComponentTransitionException.StateComponentExceptionCode.FAILEDTO_WRITEEVENTLOGBUTISPRESENT);
        	sfLog().error(e);
        }
