...
/**
 * The service type represents a service advertisement's type. They may be of three types : - simple type    :
 * 'service:simpletype' e.g. 'service:http' , 'service:telnet' , 'service:smartfrog.demo'. - abstract type  :
 * 'service:abstract-type-name:concrete-type-name' e.g. 'service:login:telnet'. - any URL scheme : 'http:' for example.
 *
...
public class ServiceType implements java.io.Serializable {
    ...
    /**
     * Create a service type object from the type name. The name may take the form of any valid service type name.
     * Type-name may be  : simple (ex --> service:http[.na]) abstract (ex--> service:login[.na]:ftp) or standard url
     * scheme, in which case it is not a service:url
     *
...
//    System.out.println("undefinedType = temporaryTypeName.substring(0,temporaryTypeName.indexOf(\".\"));" + undefinedType);
     ...
     *
     * @return a flag indicating whether the URL string which this object was constructed with was a Service URL or
     *         not.
     */
    ...
    /**
     * The principle type name, which is either the abstract type name or the protocol name, without naming authority.
     *
    ...
    /**
     * The fully formatted abstract type name, if it is an abstract type, otherwise the empty string.
     *
