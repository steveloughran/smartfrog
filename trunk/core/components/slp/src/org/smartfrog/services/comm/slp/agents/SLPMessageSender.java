...
class tcpHost {
    ...
    public int port;
    public tcpHost(InetAddress add, int port) {
        address = add;
        this.port = port;
    }
}
...
/**
    This class is used to send messages that should wait for a reply.
    These are: - SrvReq, SrvReq, SrvTypeReq, SrvDeReg.
    Objects of this class are not meant to be shared by multiple SLP agents.
    Doing that may result in strange results...
*/
class SLPMessageSender implements SlpUdpCallback {
    ...
    */
    ...
    /**
        Sends an SLP message and wait for replies.
        The SlpUdpclient used by the message sender can be shared by other 
    ...
    */
    ...
    /**
        Called when an error is found by the listener thread.
        If we get here, something is wrong with the system.
    */
    ...
    */
    ...
    /**
        Called when we are done waiting for replies.
        This stops the timer, and tells the thread to stop waiting.
    */
