...
abstract class SLPAgent implements SlpUdpCallback, SLPMessageCallbacks {
    ...
    */
    SLPAgent(Properties properties) throws ServiceLocationException {
        ...
        if( (stringVal = properties.getProperty("net.slp.DAActiveDiscoveryInterval")) != null ) {
            try {
                ...
                if(intValue == 0) CONFIG_DA_DISCOVERY = false;
                else CONFIG_DA_DISCOVERY = true;
            }catch(NumberFormatException nfe) { }
        }
        ...
        if( (stringVal = properties.getProperty("net.slp.passiveDADetection")) != null ) {
            if(stringVal.equalsIgnoreCase("true")) CONFIG_PASSIVE_DA = true;
    ...
    /**
        Does the actual work of creating the agent.
        If a list of DAs have been defined, it will contact those DAs here in
        order to get information about them. This also ensures that only DAs that
        are actually found will be added to the list of known DAs.
    */
    ...
    /**
        This method is called when the agent is to perform active DA discovery.
        The agent should then send a request for the service type "directory-agent"
    ...
    */
    ...
    /**
        Called when a DAAdvert has been received.
        If the DA is new, it is added to the list of known DAs.
        If the DA is known, and signals that it is going down 
        (timestamp == 0), it is removed from the list of DAs.
