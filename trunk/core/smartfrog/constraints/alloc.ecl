...
allocate(Cxt, Ass):-
        ...
        (var(P) ->
            writeln("Must be producers in an allocation problem"),flush(stdout),
            fail;
            preprocess_producers(P)),
        sfget(Cxt, consumers, C), 
        (var(C) ->
            writeln("Must be consumers in an allocation problem"),flush(stdout),
            fail;
            preprocess_consumers(C)),
        sfget(Cxt, colocations, Co), 
        (var(Co) ->
            true; %no problem
            preprocess_colocations(Co)),
        sfget(Cxt, noncolocations, NoCo), 
        (var(NoCo) ->
            true; %no problem
            preprocess_noncolocations(NoCo)),        
        sfget(Cxt, hosted, Hos), 
        (var(Hos) ->
            true; %no problem
            preprocess_hosteds(Hos)),        
        sfget(Cxt, default_host_cap, DHC), 
        (var(DHC) ->
            true; %no problem
            preprocess_default_host_cap(DHC)),        
        sfget(Cxt, default_vm_req, DVR), 
        (var(DVR) ->
            true; %no problem
            preprocess_default_vm_req(DVR)), 
        assign(Ass). 

preprocess_producers([]).
preprocess_producers([HP|TP]):-
        (HP=[Host,Caps] -> assert_state(host_cap(Host,Caps));
                           assert_state(host_cap(HP,nil))),
        preprocess_producers(TP).

preprocess_consumers([]).
preprocess_consumers([HC|TC]):-
        (HC=[VM,Reqs] -> assert_state(vm_req(VM,Reqs));
                           assert_state(vm_req(HC,nil))),
        preprocess_consumers(TC).
...
preprocess_default_host_cap(DHC):-
        assert_state(default_host_cap(DHC)).

preprocess_default_vm_req(DVR):-
        assert_state(default_vm_req(DVR)).

vm_reqs(V, R) :- 
        clause_state_level(vm_req(V, R)),R\==nil,!.
vm_reqs(V, R) :- 
        clause_state_level(vm_req(V, nil)), clause_state_level(default_vm_req(R)).

host_caps(H, C) :- 
        clause_state_level(host_cap(H, C)),C\==nil,!.
host_caps(H, C) :- 
        clause_state_level(host_cap(H, nil)), clause_state_level(default_host_cap(C)). 
...
host_constraints(AsList, As, VMs) :-
        ...
        (foreach(VM, VMs), count(Tick, 0, _), param(As, AsList)
         do
              ...
              get_domain_as_list(HVar, Hs),
              constrain_vm_range(As, Reqs, Exc, Hs, HsR),
              count_backtracks,
              &::(HVar,HsR),
              (var(HVar) -> indomain(HVar); true),
              retract_old_asserts(Tick),
