...
public final class FreeVar implements Copying, Cloneable, Serializable {
  ...
  /**
   * The constraint evaluation index that originates this FreeVar The constraint
   * strings in a single description get aggregated, but there are potentially
   * multiple descriptions whose aggregated constraint strings get evaluated one
   * at a time as part of link resolution. This value reflects the evaluation #
   * of the corresponding description containing this local VAR.
   */
  private int cidx = -1;
  ...
  public void setConsEvalIdx(int cidx) {
    ...
    CoreSolver.getInstance()
            .addUndo(this, ConstraintResolutionState.g_LRSUndo_PUTFVINFO);
    /*And set...*/
    this.cidx = cidx;
  }
  ...
  /**
   * Setter for attribute of the Constraint description that originates this
   * FreeVar
   *
   * @param ckey Object
   */
  ...
  /**
   * Getter for attribute of the Constraint description that originates this
   * FreeVar
   *
   * @return Object ckey
   */
  ...
  public void setTyping(Vector typeInfo) {
    if (this.typeInfo != null) {
      throw new SmartFrogEclipseRuntimeException(
              "Attempted to apply a subtyping constraint to an attribute "
                      + ckey + " which already has subtype information set");
    }
   ...
   *
   * @return ordered vector of prototype type names if typing has been previously
   *         set, otherwise null
   */
   ...
   *
   */
  public Object setRange(ComponentDescription comp)
          throws SmartFrogFunctionResolutionException {
    if (rangeRef != null) {
      ...
      try {
        ...
        throw new SmartFrogFunctionResolutionException(
                "Cannot resolve range for reference: " + rangeRef
                        + " in constraint: " + comp, e);
      }
    }
    return range;
  }
   ...
   * Constructor
   *
   * @param range  Either a reference (which will be resolved in due course),
