 ...
 *
 */
public class BalancerImpl extends ThreadedState implements Balancer, DataSource {
    //
    ...
    protected boolean requireThread()  throws StateComponentTransitionException { 
        if (asyncResponse) return false;    	
    	return true; 
    }
    		...
    		public void run(){
	      	    ...
	      	    invokeAsynchronousStateChange(new InvokeAsynchronousStateChange(){
	            	public void actOn(StateComponent _lb) {
	            		try {
	            			_lb.setTransitionToCommit(TIMER);
	            			_lb.go();
