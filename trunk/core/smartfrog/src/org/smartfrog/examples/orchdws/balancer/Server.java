...
class Server {
    ...
    private LoadBalancerBinding lbb;
     ...
     */
    Server(String name, String hostname, int port, LoadBalancerBinding lbb) {
        ...
        System.out.println("Creating new CR for:"+hostname+":"+name+":"+port+":"+lbb);
    ...
    void addNewConnection(SocketChannel clientChannel,
    	...
    	System.out.println("ADHF: Adding a new connection");
        if (!closing) {
    ...
    boolean switchLBBToUnbound(){
    	boolean toUnbound=(connections.size()==0);
    	try{ if (toUnbound) lbb.switchToUnbound();} catch (Exception e){}
    	return toUnbound;
    }
