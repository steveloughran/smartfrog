...
static void prop_print(const Prop_ptr self, FILE* file) 
{
  ...
  collateSetDumpType(COM_HP_AIL_SMARTFROG_COLLATEOUTPUT_DUMP_PROP);
  indent_node(file, "", p, " ");  //p is the prop...

  if (context != Nil) {
    collateSetDumpType(COM_HP_AIL_SMARTFROG_COLLATEOUTPUT_DUMP_IN);
    fprintf(file, "IN ");
