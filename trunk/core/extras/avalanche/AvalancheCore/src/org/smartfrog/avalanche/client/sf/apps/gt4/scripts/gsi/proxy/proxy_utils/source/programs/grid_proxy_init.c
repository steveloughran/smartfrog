...
"    -pwstdin                  Allows passphrase from command-line arguments\n" \
    ...
    char **                             argv)
{
    ...
    for(arg_index = 1; arg_index < argc; ++arg_index)
    {
        ...
        {
		//args_verify_next(arg_index, argp, "passphrase missing");
		passphrase = argv[++arg_index];
		pw_cb = globus_i_gsi_proxy_utils_pwstdin_callback;
        }
	/*else if(strcmp(argp, "-passphrase") == 0)
	{
		args_verify_next(arg_index, argp, "passphrase missing");
		passphrase = argv[++arg_index];
	}*/
