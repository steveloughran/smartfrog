...
public class MainFrame extends JFrame implements ActionListener {
    ...
    */
   public final static String version = "v0.7 r12";
...
//   private static String iniKeyClasspath = "set libs"; // Should be taken from system.property(classpath);
   ...
   private String sfDaemonDefSFFileProperty = "-Dorg.smartfrog.sfcore.processcompound.sfDefault.sfDefault=";
...
//  JTextArea jTextAreaDeploy = new JTextArea();// True means modified text.
   ...
   public static int log(String message, String who, int severity) {
       ...
       */
      String[] severityMsg = {"NONE", "FLOW", "DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"};
      StringBuffer buffer = new StringBuffer();
      int length = buffer.length();
      buffer.append(" [");
      buffer.append(new SimpleDateFormat("HH:mm:ss.SSS dd/MM/yy").format(new Date(System.currentTimeMillis())));
      buffer.append(']');
      buffer.append('[');
   ...
   public static void version() {
      System.out.println("SFGui " + org.smartfrog.tools.gui.browser.MainFrame.version);
   }
    ...
    */
   void setPreferences() {
      PreferencesDialog prefDig = new PreferencesDialog(this, "Preferences Dialog", true);
   ...
   boolean saveFile(String text) {
      ...
      try {
         ...
         JOptionPane.showMessageDialog(this, "Error saving " + currFileName, "Text Save", JOptionPane.ERROR_MESSAGE);
      }
      return false;
   }
...
/**
   ...
   boolean saveAsFile(String text) {
     while (true) {
      ...
      if (JFileChooser.APPROVE_OPTION == jFileChooser.showSaveDialog(this)) {
         ...
         if  (file.exists()){
           //-----------------------
           int value = JOptionPane.showConfirmDialog(this, "File already exist. Overwrite?",
   ...
   void jButtonAbout_actionPerformed(ActionEvent e) {
      ...
      //info();
      try {
         java.net.URL url = new java.net.URL("file:////" + System.getProperty("user.dir") + "/help/help.htm");
      ...
      if (okToAbandon(jTextAreaSFFile.getText(), this.currFileName)) {
         ...
         if (currFileName != null) {
            ...
            try {
               ...
               is = new FileInputStream(currFileName);
               jTextAreaDeploy.setText(this.parsePhase("all", is,this.getActiveLanguage()));
               is.close();
...
//                    runCmd.start();
               ...
               //log("Process Running: " + "\n   " + cmd, "RUNMnu", 3);
               log("Process Running: " + "" + auxProcess.getProcessName(), "runProcess", 3);
      ...
      if (sfFilePath != null) {
         try {
           ...
           cmdStart = cmdGeneral + " " + dir + "smartfrog" + " -a "
            ...
            auxProcess.start();
            this.processPanel.mngProcess.addProcess(auxProcess, true);
            log("Process Running: " + "" + auxProcess.getProcessName(), "runSFProcess", 3);
            this.processPanel.refresh();
            output.requestFocus();
         } catch (Exception ex) {
            log(ex.getMessage(), "runProcess", 5);
...
//      jTextAreaSFFile.grabFocus();
   ...
   JEditTextArea getActiveJTextArea() {
      ...
      } else if (indexPanel == this.jTabbedPanelNorth.indexOfTab(panelNameBrowseComp)) {
   ...
   void StopProcess() {
      try {
         this.processPanel.mngProcess.stopProcess(this.processNameTextField.getText() + "(" + this.hostNameTextField.getText() + ")");
         if (runCmd != null) {
            runCmd.kill();
            runCmd = null;
            screen.requestFocus();
...
//           +"\n  -Loaded classpath: "+classpath,"INFOMnu",3);
   ...
   void jMenuItemCleanOutput_actionPerformed(ActionEvent e) {
      int value = JOptionPane.showConfirmDialog(this, "Save Output content? ",
            "Clean Output", JOptionPane.YES_NO_CANCEL_OPTION);
      switch (value) {
          case JOptionPane.YES_OPTION:
             // yes, please save changes
   ...
   boolean saveFileOutput(String text) {
      ...
      try {
         ...
         JOptionPane.showMessageDialog(this, "Error saving " + outputFileName, "Text Save", JOptionPane.ERROR_MESSAGE);
      }
      return false;
   }
   ...
   private void setTokenEditTokenMarker(org.gjt.sp.jedit.syntax.TokenMarker tokenMarker ){
   ...
   private void loadIniFile() {
      ...
      // Not cached
      sfFilesDir = iniFile.getSetting(iniSecRunProcess, iniKeySFFilesDir, sfFilesDir);
      // Default SFSystem class
      sfSystemClass = iniFile.getSetting(iniSecRunProcess, iniKeySFSystemClass, sfSystemClass);
      //Default config for Browsers
      cmdExeBrowserLinux = iniFile.getSetting(iniSecRunProcess, iniKeyCmdExeBrowserLinux, cmdExeBrowserLinux);
      cmdExeBrowserWindows = iniFile.getSetting(iniSecRunProcess, iniKeyCmdExeBrowserWindows, cmdExeBrowserWindows);
      cmdBrowserURL = iniFile.getSetting(iniSecRunProcess, iniKeyCmdBrowserURL, cmdBrowserURL);
      // Default SF Commands ...
      cmdSFDaemon = iniFile.getSetting(iniSecRunProcess, iniKeyCmdSFDaemon, cmdSFDaemon);
      sfDaemonProcessName = iniFile.getSetting(iniSecRunProcess, iniKeySFDaemonProcessName, sfDaemonProcessName);
      sfDaemonDefIniFile = iniFile.getSetting(iniSecRunProcess, iniKeySFDaemonDefIniFile, sfDaemonDefIniFile);
      sfDaemonDefSFFile = iniFile.getSetting(iniSecRunProcess, iniKeySFDaemonDefSFFile, sfDaemonDefSFFile);
      cmdSFStart = iniFile.getSetting(iniSecRunProcess, iniKeyCmdSFStart, cmdSFStart);
      cmdSFStop = iniFile.getSetting(iniSecRunProcess, iniKeyCmdSFStop, cmdSFStop);
      // Subfix
      cmdAddSecurity = iniFile.getSetting(iniSecRunProcess, iniKeyCmdAddSecurity, cmdAddSecurity);
      lookAndFeel = iniFile.getSetting(iniSecRunProcess, iniKeyLookAndFeel, lookAndFeel);
...
//      this.processPanel.saveAll();
 ...
 //      jTabbedPaneParse.add(jTextAreaPlace, panelNamePlace);
     ...
     jPanel1.add(hostNameTextField, new GridBagConstraints(2, 0, 1, 1, 1.0, 0.0
         , GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL,
         new Insets(0, 0, 0, 0), 54, 4));
     jPanel1.add(securityCheckBox, new GridBagConstraints(3, 0, 1, 1, 0.0, 0.0
 ...
 //jFileChooser.setCurrentDirectory(new File(System.getProperty("user.dir")));
    ...
    *@created    19 September 2001
    */
   class MainFrame_jMenuItemAuto_actionAdapter implements java.awt.event.ActionListener {
      MainFrame adaptee;
    ...
    *@created    19 September 2001
    */
   class MainFrame_jMenuItemMetal_actionAdapter implements java.awt.event.ActionListener {
      MainFrame adaptee;
    ...
    *@created    19 September 2001
    */
   class MainFrame_jMenuItemWin_actionAdapter implements java.awt.event.ActionListener {
      MainFrame adaptee;
    ...
    *@created    19 September 2001
    */
   class MainFrame_jMenuItemMotif_actionAdapter implements java.awt.event.ActionListener {
      MainFrame adaptee;
    ...
    *@created    19 September 2001
    */
   class MainFrame_jMenuItemOpen_actionAdapter implements java.awt.event.ActionListener {
      MainFrame adaptee;
   ...
   protected void showLogoSequence() {
      hplogo = new ImageIcon(org.smartfrog.tools.gui.browser.MainFrame.class.getResource("hplogo.gif"));
   ...
   void scrollMsgPane() {
      ...
      try {
         this.jScrollPaneMsg.getVerticalScrollBar().setValue(this.jTextAreaMsg.getHeight() - this.jTextAreaMsg.getVisibleRect().height);
      } catch (Exception ex) {
         //e.printStackTrace();
      }
   ...
   void scrollScreenPane() {
      try {
         this.output.getVerticalScrollBar().setValue(this.screen.getHeight() - this.screen.getVisibleRect().height);
      } catch (Exception ex) {
         //e.printStackTrace();
      }
   }
   ...
   *******************************************************************/
   ...
   public final void insertPathname(String pathname) {
      ...
      if (itemnameHistory.isEmpty()) {
         ...
         for (int i=jMenuFile.getItemCount()-1, j=0; j < itemnameHistory.size(); i--, j++) {
            jMenuFile.remove(i);
         }
      }
...
//      iniFile = new IniFile(iniFileName, false);
      String pathName;
           for (int i=max_itemnames-1 ; i>=0 ; i--) {
               ...
               pathName = iniFile.getSetting(iniSecFileHistory, iniKeyFileHistory+"Name"+number , "" );
               if (!pathName.equals("")){
   ...
   public void saveHistoryIniFile() {
    ...
    if (pathnameHistory.size()>0) {
        ...
        for (i=0 ; i<this.pathnameHistory.size() ; i++) {
            ...
            if (pathName!=null){
               //if (!pathName.equals("")){
               iniFile.setSetting(iniSecFileHistory, iniKeyFileHistory+"Name"+number, pathName);
            } else {
               iniFile.setSetting(iniSecFileHistory, iniKeyFileHistory+"Name"+number, "");
            }
        }
      iniFile.flush();
   }
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemSaveAs_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemSave_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jFileChooser_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemParse_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemRun_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemCopy_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemCut_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemPaste_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemNew_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_syntaxDocumentSFFile_documentAdapter implements javax.swing.event.DocumentListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_syntaxDocumentSFFile_undoableEditAdapter implements javax.swing.event.UndoableEditListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemRedo_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemUndo_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemSelectAll_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemStop_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jTextAreaMsg_inputMethodAdapter implements java.awt.event.InputMethodListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_screen_inputMethodAdapter implements java.awt.event.InputMethodListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemInfo_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jTextAreaMsg_caretAdapter implements javax.swing.event.CaretListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_screen_caretAdapter implements javax.swing.event.CaretListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_screen_focusAdapter extends java.awt.event.FocusAdapter {
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemCleanOutput_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemCleanMsg_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_output_focusAdapter extends java.awt.event.FocusAdapter {
 ...
 *@created    19 September 2001
 */
class MainFrame_jButtonOpen_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jButtonSave_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jButtonPreferences_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jButtonUndo_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jButtonRedo_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jButtonParse_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jButtonRun_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jButtonStop_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jButtonBrowser_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemBrowseSF_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemSaveOutput_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    19 September 2001
 */
class MainFrame_jMenuItemInfoProp_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    05 December 2001
 */
class MainFrame_jButtonSFDaemon_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    05 December 2001
 */
class MainFrame_documentTextAreaMsg_documentAdapter implements javax.swing.event.DocumentListener {
 ...
 *@created    05 December 2001
 */
class MainFrame_documentScreen_documentAdapter implements javax.swing.event.DocumentListener {
 ...
 *@created    05 December 2001
 */
class MainFrame_jButtonCopy_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    05 December 2001
 */
class MainFrame_jButtonCut_actionAdapter implements java.awt.event.ActionListener {
 ...
 *@created    05 December 2001
 */
class MainFrame_jButtonPaste_actionAdapter implements java.awt.event.ActionListener {
 ...
 */
class MainFrame_jMenuItemKuntstoff_actionAdapter implements java.awt.event.ActionListener {
...
class MainFrame_jButtonSFStopDaemon_actionAdapter implements java.awt.event.ActionListener {
...
class MainFrame_jMenuItemStopSfDaemon_actionAdapter implements java.awt.event.ActionListener {
  private MainFrame adaptee;
...
class MainFrame_jMenuItemSearch_actionAdapter implements java.awt.event.ActionListener {
    MainFrame adaptee;
...
class MainFrame_jMenuItemSearchNext_actionAdapter implements java.awt.event.ActionListener {
    MainFrame adaptee;
