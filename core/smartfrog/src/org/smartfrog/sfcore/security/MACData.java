...
public class MACData {
    ...
    */
   public synchronized void addMAC(byte[] data, int start, int end) throws ShortBufferException, SecurityException {
      Mac mac = (currentMAC == null) ? defaultMAC : currentMAC;
      if (mac == null) throw new SecurityException("no key for mac calculation");
      mac.reset();
   ...
   public synchronized void checkMAC(byte[] data, int start, int end) throws SecurityException {
      if (currentMAC != null)
         if (validateMac(currentMAC, data, start, end)) return;
      if (lastMAC != null)
         if (validateMac(lastMAC, data, start, end)) return;
      if (defaultMAC != null)
         if (validateMac(defaultMAC, data, start, end)) return;
   ...
   private boolean validateMac(Mac m, byte[] data, int start, int end) {
      ...
      for (int i = 0; i < len; i++) {
         if (checkMAC[i] != data[end + 1 + i]) return false;
      }
      return true;
   }
