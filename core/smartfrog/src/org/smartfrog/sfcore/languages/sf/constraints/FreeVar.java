...
public class FreeVar implements Copying, Cloneable, Serializable {
	 ...
	 * The constraint strings in a single description get aggregated, but there are potentially multiple descriptions whose
	 * aggregated constraint strings get evaluated one at a time as part of link resolution.
	 * This value reflects the evaluation # of the corresponding description containing this local VAR.  
	 */
	private int cidx=-1;
	...
	public void setConsEvalIdx(int cidx){ 
		...
		CoreSolver.getInstance().addUndo(this, ConstraintResolutionState.g_LRSUndo_PUTFVINFO);
		/*And set...*/
		this.cidx = cidx;    	
	}
	/**
	...
	public void setTyping(Vector typeInfo) {
		if (this.typeInfo!=null) 
			throw new SmartFrogEclipseRuntimeException("Attempted to apply a subtyping constraint to an attribute "+ckey+" which already has subtype information set");
    ...
    public Object setRange(ComponentDescription comp) throws SmartFrogFunctionResolutionException { 
        if (rangeRef!=null){
        	...
        	try {
        		...
        		throw new SmartFrogFunctionResolutionException("Cannot resolve range for reference: "+rangeRef+" in constraint: "+comp, e);
        	}
        }
    	return range; 
    }
