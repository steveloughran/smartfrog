...
class EventWindowTest extends BluemineTestBase {
    ...
    public void testWindowSearch() {
        ...
        assert window.eventInWindow(s1)
        def all = window.findAll {it -> true}
        assert 1 == all.size()

        //look for in/out searches
        assert window.eventInWindow(s2)
        assert window.eventInWindow(s3)
        assert window.eventInWindow(no_name) == null
        //check the gate is in the test
        BlueEvent s4 = (BlueEvent) s3.clone()
        s4.gate = "new-gate"
        assert !window.eventInWindow(s4)
    }
