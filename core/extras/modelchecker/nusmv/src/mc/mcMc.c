...
void Mc_CheckCTLSpec(Prop_ptr prop) {
  ...
  if (bdd_is_zero(dd, s0)) { 
    ...
    /** We hack a little bit out here, which broke the model checking 
       process when running under linux.  Not essential code.
       Probably will get round to assessing cause at some point.
     */

    if (opt_counter_examples(options)) {      
      char* trace_title = " Counterexample";
	...
	exp = cons((node_ptr) bdd_dup(s0), Nil);
      ...
      collateSetDumpType(COM_HP_AIL_SMARTFROG_COLLATEOUTPUT_DUMP_FAILREC);
      ...
      collateSetDumpType(COM_HP_AIL_SMARTFROG_COLLATEOUTPUT_DUMP_NONE);
    }
  }

  collateCloseCurrentRecord();
