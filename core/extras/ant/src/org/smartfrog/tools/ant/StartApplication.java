 ...
 * <p/>
 * <i>Important</i>. The codebase set for this task in the codebase elements define the codebase
 * for interpreting the deployment descriptor, but as the action of deployment is left to a daemon,
 * JAR files and referenced in the codebase are loaded by the daemon during deployment.
 * <p/>
 * To ensure that the daemon can actually load the files, you need to
 * <ol>
 * <li>Place them in a shared location (shared file system may work)
 * <li>Place URLs to the shared files in the smartfrog deployment descriptor's
 * <tt>sfCodebase</tt> attribute. The &lt;sf-tourl&gt; task can be used to create
