...
public class SFDeployToTomcat extends PrimImpl implements Prim {
	...
	public synchronized void sfStart() throws SmartFrogException, RemoteException {
		...
		try {
			...
			sfLog().info("Failed to deploy GT4 WSCore into Tomcat");
			throw new SmartFrogException("Failed to deploy GT4 WSCore into Tomcat : " + 
					be.getMessage());
		}
		...
		try {
			if (!wscore.editXMLFiles(tomcatVersion, connAttrs, valveAttrs)) {
				sfLog().info("Failed to configure the deployment...");
				throw new SmartFrogException("Configuration failed..." + 
					"Pls check log files for more details.");
			}
		} catch (TomcatConfigException tce) {
			sfLog().info("TomcatConfigException : " + tce);
