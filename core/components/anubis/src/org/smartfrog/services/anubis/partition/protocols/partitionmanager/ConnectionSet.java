 ...
 * @version 1.0
 */
public class ConnectionSet extends CompoundImpl
        implements Compound, ViewListener, HeartbeatReceiver {
    ...
    public void sfDeploy() throws RemoteException, SmartFrogException {
        ...
        try {
             ...
             */
            heartbeatProtocolFactory = (HeartbeatProtocolFactory) sfResolve("heartbeatProtocolFactory");
            leaderProtocolFactory = (LeaderProtocolFactory) sfResolve("leaderProtocolFactory");
            leaderMgr = leaderProtocolFactory.createMgr(connections, self);

            /**
             * Timeouts
             */
            heartbeatInterval = Config.getLong(this, "heartbeatInterval");
            long timeoutCount = Config.getLong(this, "heartbeatTimeout");
             ...
             */
            intervalExec = new IntervalExec(me, this, heartbeatInterval);

            /**
             * Heartbeat message initialisation
             */
            heartbeat = heartbeatProtocolFactory.createMsg( me, connectionServer.getAddress() );
    ...
    public void sfStart() throws RemoteException, SmartFrogException {
        ...
        if (log.isInfoEnabled()) {
            log.info(me + " connection address is " + connectionServer.getAddress().toString());
        }

        connectionView.setTimeStamp( me.epoch );
        intervalExec.start();
    ...
    public synchronized boolean receiveHeartbeat(Heartbeat hb) {
        ...
        if( con == null ) {
            ...
            con = new HeartbeatConnection(me, this, hbp, can);
            addConnection(con);
    ...
    public synchronized void convertToMessageConnection(HeartbeatConnection con) {
        ...
        if( thisEndInitiatesConnectionsTo(con.getSender()) ) {
            MessageConnection mcon = new MessageConnection(me, this, con.getProtocol(), con.getCandidate());
            ...
            getConnectionServer().initiateConnection(me, mcon, heartbeat);
        }
    }
    ...
    public synchronized void convertToHeartbeatConnection(MessageConnection mcon) {
        Connection con = new HeartbeatConnection(me, this, mcon.getProtocol(), mcon.getCandidate());
    ...
    private boolean consistent(long timenow) {
        ...
        while( iter.hasNext() ) {
             ...
             * stability time
             */
            if( connectionView.contains(con.getSender()) && !con.getSender().equalId(me) ) {
                /**
    ...
    public synchronized MessageConnection connect(int id) {
        ...
        MessageConnection mcon = new MessageConnection(me, this, hbcon.getProtocol(), hbcon.getCandidate());
