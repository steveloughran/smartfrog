...
public class LocalProviders {
    ...
    private class ProviderInfo {
        ...
        public ProviderInfo(AnubisProvider provider) {
            this.proxy = new ProviderProxy(provider.getName(), me);
        }

        public String toString() {
            String str = "    " + proxy + " has "
    ...
    public synchronized void register(AnubisProvider provider, ValueData value, long time) {
         ...
         */
        ProviderInstance instance = new ProviderInstance(provider.getName(), provider.getInstance(), me, time, value);
    ...
    public synchronized void newValue(AnubisProvider provider, ValueData value, long time) {
        /**
    ...
    public synchronized void deregister(AnubisProvider provider) {
        ...
        Iterator iter = info.listeners.values().iterator();
        while( iter.hasNext() )
            locator.sendToLocal(RegisterMsg.providerNotPresent(instance), ((ListenerProxy)iter.next()).node);
    ...
    public synchronized void addListener(ListenerProxy listener) {
        ...
        if( !providers.containsKey(listener.name) ) {
            ...
            if( log.isTraceEnabled() ) log.trace(me + ": no provider info matching reported " + listener);
            return;
        ...
        if( existingReg == null ) {
            ...
            if( log.isTraceEnabled() ) log.trace(me + ": new registration " + listener);
         ...
         */
            ...
            if( log.isTraceEnabled() ) log.trace(me + ": new reg superceeds existing registration " + listener);
         ...
         */
            ...
            if( log.isTraceEnabled() ) log.trace(me + ": new reg does not superceed existing registration " + listener);
            return;
        ...
        if( log.isTraceEnabled() ) log.trace(me + ": sending states of " +
        ...
        while( iter.hasNext() ) {
            ...
            if( log.isTraceEnabled() ) log.trace(me + ": sending " + msg + " to node " + listener.node);
    ...
    public synchronized void removeListener(ListenerProxy listener) {
         ...
         */
        if( !providers.containsKey(listener.name) ) {
            if( log.isTraceEnabled() ) log.trace(me + ": no provider info matching removed " + listener);
            return;
        }
        ...
        if( listener.uridPreceeds(existingReg) ) {
            if( log.isTraceEnabled() )
                log.trace(me + " existing registration superceeds deregistration " + listener);
            return;
        }
        ...
        if( log.isInfoEnabled() ) log.info(me + ": provider info removed " + listener);
    }
    ...
    public synchronized String toString() {
        ...
        while( iter.hasNext() ) {
            ...
            str += "    " + node;
            for(Iterator iter2=listenersByNode.getSet(node).iterator();
                iter2.hasNext();
                str += " " + ((ListenerProxy)iter2.next()).name);
            str += "\n";
        }
