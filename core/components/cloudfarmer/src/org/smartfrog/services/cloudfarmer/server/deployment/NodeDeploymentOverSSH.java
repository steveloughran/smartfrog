...
final class NodeDeploymentOverSSH extends AbstractNodeDeployment implements NodeDeploymentService {
    ...
    NodeDeploymentOverSSH(NodeDeploymentOverSSHFactory factory, ClusterNode node) {
        super(node);
    ...
    public synchronized void deployApplication(String name, ComponentDescription cd)
        ...
        if(!getClusterNode().isExternallyVisible()) {
            factory.sfLog().warn("This node is not externally visible. Unless we are in the cell, deployment will not work");
        }
        ...
        factory.getLog().info("Deploying application "+ name + " to " + connectionDetails);
    ...
    public boolean terminateApplication(String name, boolean normal, String exitText)
        ...
        Session session = null;
        try {
            factory.getLog().info("Terminating application " + name + " for " + exitText);
            session = factory.demandCreateSession(hostname);
            ArrayList<String> commandsList = new ArrayList<String>(1);
