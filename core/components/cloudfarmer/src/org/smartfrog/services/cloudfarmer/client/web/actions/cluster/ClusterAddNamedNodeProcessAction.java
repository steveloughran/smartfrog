...
public class ClusterAddNamedNodeProcessAction extends AbstractClusterAction {
    ...
    protected String getActionName() {
        return "ClusterAddNamedNodeProcessAction";
    }
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
        ...
        log.info("Adding a named node " + hostname);
        try {
            int hc1 = controller.getHostCount();
            controller.addNamedHost(hostname, addForm.isMaster(), addForm.isWorker());
            int hc2 = controller.getHostCount();
            if (hc1 == hc2) {
                //this verifies that the controller is actually working, otherwise
                //the request is effectively a failure
                return failure(request, mapping, "The controller " + controller.getDescription()
                        + " did not add the new host");
            }
            addClusterAttributes(request, controller);
            return success(mapping);
        } catch (Exception e) {
            return failure(request, mapping, "Failed to add the named host " + hostname + " : " + e, e);
        }
